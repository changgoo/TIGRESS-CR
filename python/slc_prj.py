...
class SliceProj:
    ...
    def set_prj_dfi(self):
        ...
        prjkwargs["mflux"] = dict(
            norm=SymLogNorm(1.0e-4, vmin=-1.0e-1, vmax=1.0e-1), cmap=cmr.fusion_r
        )
        prjkwargs["mZflux"] = prjkwargs["mflux"]
        prjkwargs["teflux"] = dict(
            norm=SymLogNorm(1.0e40, vmin=-1.0e46, vmax=1.0e46), cmap=cmr.viola
        )
        ...
        labels["keflux"] = (
            r"$\mathcal{F}_{e_{\rm kin}}\,[{\rm erg\,kpc^{-2}\,yr^{-1}}]$"
        )
        labels["creflux"] = (
            r"$\mathcal{F}_{e_{\rm cr}}\,[{\rm erg\,kpc^{-2}\,yr^{-1}}]$"
        )
        labels["creflux_diff"] = (
            r"$\mathcal{F}_{e_{\rm cr},{\rm diff}}\,[{\rm erg\,kpc^{-2}\,yr^{-1}}]$"
        )
        labels["creflux_adv"] = (
            r"$\mathcal{F}_{e_{\rm cr},{\rm adv}}\,[{\rm erg\,kpc^{-2}\,yr^{-1}}]$"
        )
        labels["creflux_str"] = (
            r"$\mathcal{F}_{e_{\rm cr},{\rm str}}\,[{\rm erg\,kpc^{-2}\,yr^{-1}}]$"
        )
    ...
    def get_prj(
        ...
        if "vel3" in data:
            ...
            prjdata["teflux"] = (
                gamma / (gamma - 1) * data["press"] * data["vel3"] * conv_eflux
            )
            prjdata["keflux"] = (
                0.5
                * data["rho"]
                * data["vel3"]
                * (data["vel1"] ** 2 + data["vel2"] ** 2 + data["vel3"] ** 2)
                * conv_eflux
            )
            if "rmetal" in data:
                prjdata["mZflux"] = (
                    data["rho"] * data["rmetal"] * data["vel3"] * conv_mflux
                )
        ...
        if self.options["cosmic_ray"]:
            ...
            if "0-Vd3" in data:
                prjdata["creflux_diff"] = (
                    data["0-Ec"] * data["0-Vd3"] * conv_eflux * 4 / 3.0
                )
                prjdata["creflux_adv"] = (
                    data["0-Ec"] * data["vel3"] * conv_eflux * 4 / 3.0
                )
                prjdata["creflux_str"] = (
                    data["0-Ec"] * data["0-Vs3"] * conv_eflux * 4 / 3.0
                )
        if self.options["newcool"]:
            prjdata["Sigma_HI"] = data["rho"] * data["rHI"] * conv_surf
            prjdata["Sigma_H2"] = 2 * data["rho"] * data["rH2"] * conv_surf
            prjdata["Sigma_HII"] = (
                data["rho"] * (1 - data["rHI"] - 2 * data["rH2"]) * conv_surf
            )
