...
def plot_slice_xy(
    ...
    # if field == "Fcr3":
    #     data = np.abs(data)
    lunit_factor = 1.0e-3 if kpc else 1
    im = plt.pcolormesh(
        data.x * lunit_factor,
        data.y * lunit_factor,
        data,
...
def plot_slices(sim, num, savefig=True):
    ...
    for ax, field, vec in zip(axes[1, :], flist, vectors):
        ...
        im = plot_slice_xy(
            ...
            vec=vec,
            stream_kwargs=dict(
                density=0.5, color="w" if field in ["Bmag", "pok_mag"] else "k"
            ),
        )
        if im is not None:
            ax.set_aspect("equal", adjustable="box")
    ...
    for i, phase in enumerate(phlist):
        for ax, field, vec in zip(axes[i::nphase], flist, vectors):
            ...
            try:
                ...
                continue
            im = plt.pcolormesh(prj.x, prj.z, prj, **prjkwargs[field])
            if im is not None:
                cbar = plt.colorbar(
                    im,
                    orientation="horizontal",
...
def plot_snapshot(
    ...
    for i, (ax, f) in enumerate(zip(g1, fields_xy)):
        ...
        if f in prj_xy:
            ...
            im = plt.pcolormesh(prj.x, prj.y, prj, **prjkwargs[f])
        else:
    ...
    for i, (ax, f) in enumerate(zip(g2, fields_xz)):
        ...
        if f in prj_xz:
            ...
            im = plt.pcolormesh(prj.x, prj.z, prj, **prjkwargs[f])
            label = labels[f]
        else:
    ...
    for k, (model, num, parnum, xygrid, xzgrid) in enumerate(
        ...
        for i, (ax, f) in enumerate(zip(xygrid, fields_xy)):
            ...
            if f in prj_xy:
                prj = prj_xy[f].sel(phase="whole")
                im = plt.pcolormesh(
                    prj.x * lfactor, prj.y * lfactor, prj, **prjkwargs[f]
                )
                label = labels[f]
            else:
            ...
            if f in sink_fields:
                ...
                colorbar_sp(
                    fig1, 40, bbox=bbox, orientation="horizontal", tickloc="top"
                )
            ax.set(xlim=(le[0], re[0]), ylim=(le[1], re[1]))
            if kpc:
        ...
        for i, (ax, f) in enumerate(zip(xzgrid, fields_xz)):
            ...
            if f in prj_xz:
                prj = prj_xz[f].sel(phase="whole")
                im = plt.pcolormesh(
                    prj.x * lfactor, prj.z * lfactor, prj, **prjkwargs[f]
                )
                label = labels[f]
            else:
...
def plot_slices_cr(
    ...
    for ax, field in zip(axes[0, :], flist):
        ...
        if im is not None:
            ...
            cbar.set_label(
                label=sim.dfi[field]["label_name"]
                + "\n"
                + sim.dfi[field]["label_unit"],
                size="large",
            )
            ax.set_aspect("equal", adjustable="box")
        ax.axis("off")
